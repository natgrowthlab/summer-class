...
async function createLiquidationRequest(studentId, receiptUrl) {
  ...
  try {
    await pool.query(
      'INSERT INTO bonus_liquidations (id,talonario_id,student_id,amount,receipt_url) VALUES (?,?,?,?,?)',
      [id, ticket.id, studentId, ticket.available_amount, receiptUrl]
    );
  } catch (error) {
    if (error.code === '23505') return { error: 'Ya tienes una liquidación pendiente para este talonario.' };
    throw error;
  }
  return { liquidation: { id, talonario_id: ticket.id, amount: ticket.available_amount, receipt_url: receiptUrl, status: 'pending' }, ticket };
}
